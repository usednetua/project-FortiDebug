"""Application Debug — diagnose debug application <daemon>."""
...
class AppDebugTab(BaseTab):
    ...
    def _build_ui(self):
        ...
        warn = ctk.CTkLabel(
            self,
            text="⚠ Real-time app debug — CPU intensive. Stop after capture.",
            text_color="#e67e22",
        )
        warn.grid(row=1, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 8))

        ctk.CTkLabel(self, text="Daemon").grid(row=2, column=0, sticky="w", padx=10, pady=4)
        ...
        self.daemon.grid(row=2, column=1, sticky="ew", padx=10, pady=4)

        self.hint = ctk.CTkLabel(self, text=self.DAEMONS["authd"], text_color="gray")
        self.hint.grid(row=3, column=0, columnspan=2, sticky="w", padx=10, pady=2)

        ctk.CTkLabel(self, text="Debug level").grid(row=4, column=0, sticky="w", padx=10, pady=4)
        self.level = ctk.CTkOptionMenu(
            self,
            values=["-1 (all)", "0", "1", "2", "3", "4", "7", "255"],
            command=lambda _: self.notify_change(),
        )
        self.level.set("-1 (all)")
        self.level.grid(row=4, column=1, sticky="ew", padx=10, pady=4)
        ...
        self.duration.grid(row=5, column=1, sticky="ew", padx=10, pady=4)
        ...
        self.reset.select()
        self.reset.grid(row=6, column=0, columnspan=2, sticky="w", padx=10, pady=4)

        self.timestamps = ctk.CTkCheckBox(
            self, text="Console timestamps", command=self.notify_change
        )
        self.timestamps.select()
        self.timestamps.grid(row=7, column=0, columnspan=2, sticky="w", padx=10, pady=2)

        self.debug_info = ctk.CTkCheckBox(
            self, text="diagnose debug info", command=self.notify_change
        )
        self.debug_info.grid(row=8, column=0, columnspan=2, sticky="w", padx=10, pady=2)
