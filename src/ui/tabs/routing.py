"""Routing tab — OSPF, BGP, Static/RIB."""
...
class RoutingTab(BaseTab):
    ...
    def _build_ui(self):
        ...
        self.ospf_status = ctk.CTkCheckBox(self, text="Status / neighbors / interfaces", command=self.notify_change)
        ...
        self.ospf_debug = ctk.CTkCheckBox(self, text="Live OSPF debug (verbose)", command=self.notify_change)
        ...
        self.bgp_routes = ctk.CTkCheckBox(self, text="Routes / advertised-routes (needs neighbor)", command=self.notify_change)
        self.bgp_routes.grid(row=8, column=0, columnspan=2, sticky="w", padx=10, pady=2)

        self.bgp_network = ctk.CTkCheckBox(self, text="Locally originated networks", command=self.notify_change)
        ...
        self.rib_db = ctk.CTkCheckBox(self, text="RIB database (incl. hidden)", command=self.notify_change)
        ...
        self.stop_debug = ctk.CTkCheckBox(self, text="Append stop-debug block", command=self.notify_change)
        self.stop_debug.select()
        self.stop_debug.grid(row=17, column=0, columnspan=2, sticky="w", padx=10, pady=6)
    ...
    def generate_commands(self) -> str:
        ...
        if self.ospf_status.get():
            lines.append("get router info ospf status")
            lines.append("get router info ospf neighbor")
            lines.append("get router info ospf interface")
        if self.ospf_lsdb.get():
            lines.append("get router info ospf database")
            lines.append("get router info routing-table ospf")
        ...
        if self.proute.get():
            lines.append("diagnose firewall proute list")
            lines.append("diagnose firewall proute6 list")

        lookup = self.lookup.get().strip()
        if lookup:
            lines.append(f"get router info routing-table details {lookup}")

        if (self.ospf_debug.get() or self.bgp_debug.get()) and self.stop_debug.get():
            lines.append("")
            lines.append("# --- then: ---")
            lines.append("diagnose debug disable")
            lines.append("diagnose ip router ospf all disable")
            lines.append("diagnose ip router bgp all disable")
