...
class HaTab(BaseTab):
    ...
    def _build_ui(self):
        ...
        self.ha_status = ctk.CTkCheckBox(self, text="get system ha status + diagnose sys ha status", command=self.notify_change)
        ...
        self.full_dump = ctk.CTkCheckBox(self, text="Full HA dump (group + vcluster)", command=self.notify_change)
        ...
        self.force_sync = ctk.CTkCheckBox(self, text="Force full sync (ha synchronize all)", command=self.notify_change)
        self.force_sync.grid(row=8, column=0, columnspan=2, sticky="w", padx=10, pady=2)

        self.reset_uptime = ctk.CTkCheckBox(self, text="Reset HA uptime (may trigger failover!)", command=self.notify_change)
        ...
        self.stop_debug.grid(row=15, column=0, columnspan=2, sticky="w", padx=10, pady=4)
    ...
    def generate_commands(self) -> str:
        ...
        if idx:
            ...
            if admin:
                lines.append(f"execute ha manage {idx} {admin}")
            else:
                lines.append(f"execute ha manage {idx}")
